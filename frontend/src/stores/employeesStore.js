...
export const useEmployeesStore = defineStore('employees', {
    ...
    state: () => ({
        ...
        filterDate: '',
        selectedFilter: '',
        updatedData: localStorage.getItem('updatedData') ? JSON.parse(localStorage.getItem('updatedData')) : null,
        dataChanged: localStorage.getItem('dataChanged') ? JSON.parse(localStorage.getItem('dataChanged')) : false,
        message: null,
        newEmployee: {
            first_name: '',
            last_name: '',
    ...
    actions: {
        ...
        searchEmployees(searchTerm) {
            axios.get(`/api/employees?search=${searchTerm}`)
                .then((response) => {
                    this.employees = response.data;
                });
        },
        ...
        createSkill(employeeId, skillData) {
            axios.post(`/api/skills/`, { name: skillData.name, yrs_exp: skillData.yrs_exp, seniority: skillData.seniority, employee: employeeId })
                .then((response) => {
                    console.log(`Skill created successfully with ID: ${response.data.id}`);
                });
        },
        unpdateSkill(skillId, skillData) {
            axios.put(`http://127.0.0.1:8000/api/skills/${skillId}/`, skillData).then(() => {
        ...
        deleteSkill(skillId) {
            axios.delete(`http://127.0.0.1:8000/api/skills/${skillId}/`).then(() => {
